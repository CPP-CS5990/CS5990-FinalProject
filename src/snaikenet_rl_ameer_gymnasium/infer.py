...
def infer(
    ...
    for episode in range(1, num_episodes + 1):
        ...
        food_visible_pct = (
            (food_visible_steps / (step + 1)) * 100.0 if step + 1 > 0 else 0.0
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run a trained DQN agent on SnaikeNET (inference only)"
    )
    parser.add_argument(
        "--model", type=str, required=True, help="Path to checkpoint (.pt file) to load"
    )
    parser.add_argument("--host", type=str, default="localhost", help="Server host")
    parser.add_argument("--port", type=int, default=8888, help="Server TCP port")
    parser.add_argument(
        "--episodes", type=int, default=10, help="Number of inference episodes"
    )
    parser.add_argument(
